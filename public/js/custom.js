...
/* Contact Form */
	...
	$('#send_message').click(function(e){
        ...
        if(error === false){
            ...
            $.post("js/php/send_email.html", $("#contact_form").serialize(),function(result){
				if(result === 'sent'){
					$('#errors').hide();
					$('#send_message').remove();
...
/**************************** Footer contact Form ***************************/
	...
	$('#send').click(function(e){
        ...
        if(error === false){
            ...
            function serialize() which gets all the data from the form
            we submit it to send_email.php */
            $.post("js/send.html", $("#footermsg").serialize(),function(result){
				if(result === 'sent'){
					$('#errors').hide();
					$('#send').remove();
                    $('#mail_s').fadeIn(500);
	...
	function ajax_newsletter() {
		...
		$.ajax({
			...
			success: function(msg) {
				$('#response').html('You has been Subscribe');
			}
		});
		return false;
